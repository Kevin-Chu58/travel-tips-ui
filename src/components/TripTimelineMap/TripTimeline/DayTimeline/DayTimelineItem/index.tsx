...
const DayTimelineItem = React.memo(
  ({
    i,
    j,
    day,
    ...
    useEffect(() => {
      if (j + 1 < (day.tripAttractionOrders?.length ?? 0)) {
        const nextTao = day.tripAttractionOrders!.at(j + 1);
        setNextTao(nextTao);
      }
    }, [day]);
    ...
    // rerender the cummulatedTime of the day on cummulatedTimes
    useEffect(() => {
      if (dayAcummulatedTimes[i]) {
        setCummulatedTime(dayAcummulatedTimes[i][j]);
      }
    }, [dayAcummulatedTimes]);
    ...
    const isFocus = tao.id.toString() === mapFocusId;
    ...
    const TimelineItemBase = () => {
      return (
        ...
        >
          ...
          <TimelineSeparator>
            <TimelineDot />
            {j < day.tripAttractionOrders!.length - 1 && <TimelineConnector />}
          </TimelineSeparator>
          <TimelineContent flexGrow={1} mb={2} sx={{ minWidth: "75%" }}>
            <Grid size={12} spacing={1}>
                ...
                <Box sx={{ position: "absolute", top: 2, right: 2 }}>
                  <NonBlockingPopover>
                    <List disablePadding>
                      <ListItemButton
                        disableRipple
                        onClick={() => updateOpenEditTao(tao.id, j)}
                      >
                        <ListItemIcon>
                          <EditIcon />
                        </ListItemIcon>
                      ...
                      <ListItemButton disableRipple onClick={handleDeleteRoute}>
                        <ListItemIcon>
                          <DeleteIcon />
                        </ListItemIcon>
                        <ListItemText
              ...
              {/* routes */}
              {j < day.tripAttractionOrders!.length - 1 && (
                <TTCard
                  bgcolor="steelblue"
                  darkBg={true}
                  icon={
    ...
    const isEditFocus = !readonly && isFocus;
    ...
    return (
      ...
      >
        ...
        >
          <AddTaoButton
            onClick={() => updateOpenEditTao(undefined, j)}
            sx={{
              position: "absolute",
              top: -16,
              ml: "auto",
              zIndex: 5,
              display: isEditFocus ? "auto" : "none",
            }}
          />
          <TimelineItemBase />
          <AddTaoButton
            onClick={() => updateOpenEditTao(undefined, j + 1)}
            sx={{
              position: "absolute",
              bottom: -16,
              zIndex: 5,
